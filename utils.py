...
class EnhancedFeatureExtractor:
    def __init__(self):
        ...
        # --- 定义本地模型路径 ---
        self.local_paths = {
            'vit': './resources/models/vit-base',
            'bert': './resources/models/bert-base',
            'clip': './resources/models/clip-base'
        }

        try:
            # 优先使用本地路径，如果不存在则使用云端 ID
            vit_path = self.local_paths['vit'] if os.path.exists(
                self.local_paths['vit']) else "google/vit-base-patch16-224"
        ...
        try:
            bert_path = self.local_paths['bert'] if os.path.exists(self.local_paths['bert']) else "bert-base-uncased"
            print(f"Loading BERT from: {bert_path}")

            self.bert_model = BertModel.from_pretrained(bert_path).to(self.device)
        ...
        try:
            clip_path = self.local_paths['clip'] if os.path.exists(
                self.local_paths['clip']) else "openai/clip-vit-base-patch32"
